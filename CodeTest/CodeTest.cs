...
public readonly struct CodeTest
{
    ...
    )
    {
        ...
        for (int i = 0; i < Code.Count; i++)
        {
            ...
            {
                await logger.LogTreeAsync(tree, $"Test code {i}", cancellationToken);
            }
        ...
        if (generatorResults.Count > 0)
        {
            foreach (KeyValuePair<Type, GeneratorDriver> generatorResult in generatorResults)
            {
                ...
                if (treesCount > 0)
                {
                    ...
                    {
                        await logger.LogTreeAsync(tree, $"Generated by ({generatorResult.Key.FullName})", cancellationToken);
                    }
                }
            }
        }
